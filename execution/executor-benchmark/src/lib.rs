...
mod tests {
    ...
    // Note: The old test_compare_perpdex_move_and_native test that compared
    // PerpDexNativeVMBlockExecutor against AptosVMBlockExecutor has been removed.
    // Native execution is now done via NATIVE_DISPATCH=1 env var which intercepts
    // at the entry function level inside AptosVM itself. Correctness is validated
    // by running the perpdex-benchmark with both NATIVE_DISPATCH=0 and =1 and
    // comparing event counts (all 11 event types must match exactly).
}
