...
struct ModuleSourceData {
    ...
    /// Maps `FileHash` → file metadata (name + line starts).
    files: HashMap<FileHash, FileData>,
}
...
pub struct AptosSourceLocator {
    source_data: HashMap<ModuleId, ModuleSourceData>,
    /// module_id → struct_name → ordered field names.
    struct_field_names: HashMap<ModuleId, HashMap<String, Vec<String>>>,
}
...
impl AptosSourceLocator {
    ...
    /// `source_filename` – the display filename (e.g. `"sources/perp.move"`).
...
impl SourceLocator for AptosSourceLocator {
    fn locate(
        ...
        let debug = std::env::var("MOVE_SOURCE_LOCATOR_DEBUG").is_ok();
        ...
        let data = match self.source_data.get(module_id) {
            ...
            None => {
                if debug {
                    eprintln!(
                        "[source_locator] no source data for module {} (have: {:?})",
                        module_id,
                        self.source_data.keys().collect::<Vec<_>>()
                    );
                }
                return None;
            },
        ...
        let file_data = match data.files.get(&loc.file_hash()) {
            ...
            None => {
                if debug {
                    eprintln!(
                        "[source_locator] file hash {:?} not found for {} (have: {:?})",
                        loc.file_hash(),
                        module_id,
                        data.files.keys().collect::<Vec<_>>()
                    );
                }
                return None;
            },
