...
module aptos_framework::ethereum_derivable_account {
    ...
    const EUNAUTHORIZED_DOMAIN: u64 = 6;
    ...
    public fun authenticate(account: signer, aa_auth_data: AbstractionAuthData): signer acquires AuthorizedDomains {
    ...
    #[test(framework = @0x1)]
    fun test_authenticate_auth_data(framework: &signer) acquires AuthorizedDomains {
        chain_id::initialize_for_test(framework, 4);
    ...
    fun test_authenticate_auth_data_invalid_signature(framework: &signer) acquires AuthorizedDomains {
        chain_id::initialize_for_test(framework, 4);
