...
//! Native implementation for box::borrow_storage_slot_resource and box::borrow_storage_slot_resource_mut
//!
...
/***************************************************************************************************
 * native fun borrow_storage_slot_resource<T: store, BR>(self: &Box<T>): &BR
 *
 * Borrows BoxedResource<T> from global storage.
 * Called from Move as: borrow_storage_slot_resource<T, BoxedResource<T>>(box)
 *
...
/***************************************************************************************************
 * native fun borrow_storage_slot_resource_mut<T: store, BR>(self: &mut Box<T>): &mut BR
 *
 * Borrows BoxedResource<T> mutably from global storage.
 * Called from Move as: borrow_storage_slot_resource_mut<T, BoxedResource<T>>(box)
 *
 * Type args: [T, BoxedResource<T>]
 *   - ty_args[0] = T (the value type)
 *   - ty_args[1] = BoxedResource<T> (the resource type to borrow from global storage)
...
pub fn make_all(
    ...
    let natives = [
        ("borrow_storage_slot_resource", native_borrow_storage_slot_resource as RawSafeNative),
        ("borrow_storage_slot_resource_mut", native_borrow_storage_slot_resource_mut),
    ];
