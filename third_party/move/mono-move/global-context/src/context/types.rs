...
/// Checks equivalence between a [`Type`] and a [`SignatureToken`] struct or
/// enum (identified by
/// handle index and type arguments) and an interned [`Type`] node.
///
/// # Safety
///
...
impl PartialEq for TypeListInternerKey {
    fn eq(&self, other: &Self) -> bool {
        ...
        this.iter()
            .zip(other.iter())
            .all(|(this, other)| TypeInternerKey(*this) == TypeInternerKey(*other))
    }
}
