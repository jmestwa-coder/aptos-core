...
```
                      aptos-core                 aptos-networks         mainnet-proposals (Foundation)
                     ────────────               ────────────────       ──────────────────────────────
Testnet:
  1. Checkout release branch
  2. Generate gas schedule JSON  ──────────────> 3. PR gas snapshot
  4. Edit release.yaml (endpoint, gas refs, name)
  5. PR release.yaml (2 reviewer approvals)
  6. Run generate-proposals --simulate testnet
  7. Trigger release-framework GH Action (internal-ops)
  8. Verify on-chain (Grafana, Explorer)
  9. Push framework tag

Mainnet:
  1. Run generate-proposals from release branch
  2. Run simulate --network mainnet
  3. Fork Foundation repo, copy artifacts  ────────────────────────> 4. PR metadata + sources
  5. Coordinate with operator (#labs-operator-voting) to create governance proposal
  6. Submit proposal link to Foundation (#foundation-governance) → 3-day voting window
  7. Execute proposal (operator or self-serve)
  8. Verify on-chain (Explorer: PackageRegistry / GasScheduleV2)
  9. Push mainnet branch (internal-ops workflow)
  10. Load testing (before + after)
```
...
particularly important gas parameters (e.g., `max_execution_gas`, `max_io_gas`,
`max_transaction_size_in_bytes`, and others defined in `transaction.rs`), the tooling
could generate additional per-parameter checkboxes to force explicit acknowledgment.
...
- Gas `old.json` / `new.json` match version references in release.yaml
